...
/*
 *  workaround: require 3D response when no 3C request.
 *  @param [in] nad: the NAD required.
 *  @return none
 */
void ls_set_init_req_nad(uint8_t nad);
/*
...
/*
 *  this api only take effects followed by ls_set_nad()for ls_set_nad() would rewrite broadcaset fids based on the specific nad.
 *  the broadcaset fids would be fixed ignore specific DNN nad 
 *  @param [in] fid0: 0x38/0x39
 *  @param [in] fid1: 0x3A/0x3B
 *  @return none
 */
void ls_set_j2602_customized_dnn_broadcast_fid(l_u8 fid0,l_u8 fid1);
