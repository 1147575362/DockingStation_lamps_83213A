...
void LINS_Init(void)
{
    ...
    LINS_SFRS->CTRL.SLEEP = 0U;/*  wake up lins when init if sleep  */
    ...
    PMUAPRE5V_SFRS->LINS.LINS_PU30K_ENA = 1U;     /* LIN 30K pullup enable.*/
    IOCTRLA_SFRS->LIN.LINS_TXENA        = 1U;     /* LIN transmit enable.*/
    PMUAPRE5V_SFRS->LINS.LINS_RXENA     = 1U;     /* LIN receive enable. */
    ...
    PMUAPRE5V_SFRS->LINS.LINS_TX_BIAS_BOOST = (uint8_t)HWCFG_GetLINS_TxBiasBoost();
    PMUAPRE5V_SFRS->LINM.LINM_TX_BIAS_BOOST = (uint8_t)HWCFG_GetLINM_TxBiasBoost();    
    PMUAPRE5V_SFRS->LINS.LINS_RX_BIAS_BOOST = 0x03U;
    PMUAPRE5V_SFRS->LINM.LINM_RX_BIAS_BOOST = 0x03U;

#if AUTO_ADDRESSING_EN == 0U
    PMUAPRE5V_SFRS->LINS.SWON             	= 0U; /* 1: LIN slave switch On between LIN_IN and LIN_OUT pin*/

    PMUAPRE5V_SFRS->LINS.LIN_TX_S_SL 		= (uint8_t)HWCFG_GetLINS_SlewRate();
    PMUAPRE5V_SFRS->LINS.LIN_TX_S_DT 		= (uint8_t)HWCFG_GetLINS_TxDuty();
#else
    PMUAPRE5V_SFRS->LINM.LIN_TX_M_SL        = 0x0F;
    PMUAPRE5V_SFRS->LINS.LIN_TX_S_DT        = 0x02U;    
    PMUAPRE5V_SFRS->LINS.LIN_TX_S_SL        = 0x0F;
    PMUAPRE5V_SFRS->LINM.LIN_TX_M_DT        = 0x02U;  

    PMUAPRE5V_SFRS->LINS.SWON               = 1U; /* 1: LIN slave switch On between LIN_IN and LIN_OUT pin*/
    PMUAPRE5V_SFRS->LINM.LINM_PUOFF_TIMEOUT = 0U;
#endif
    ...
    /* Enable wakeup detect and IRQ  when LINS is in sleep mode(LINS_SFRS->CTRL..SLEEP = 1U;)  */
...
{
   PMUAPRE5V_SFRS->LINS.SWON = (uint8_t)state;
}
...
void LINM_Init(void)
{
    ...
    PMUAPRE5V_SFRS->LINM.LINM_RXENA             = 1U;    /* LIN receive enable. */
    IOCTRLA_SFRS->LIN.LINM_TXENA                = 1U;    /* LIN transmit enable.*/
    PMUAPRE5V_SFRS->LINM.LINM_PU1K_ENA          = 1U;    /* LIN 1K pullup enable.*/
    PMUAPRE5V_SFRS->LINS.SWON                   = 0U;    /* 1: disable Master function, 0: enable Master function on LINOUT pin */
    PMUAPRE5V_SFRS->LINM.LINM_PUOFF_TIMEOUT     = 0U;    

    LINM_SFRS->BUSTIME.BUSINACTIVE      = E_LIN_TIME_INACTIVE_SEC_4;
    LINM_SFRS->BUSTIME.WUPREPEAT        = E_LIN_TIME_WAKEUP_REPEAT_MS_180;
    LINM_SFRS->CTRL.RST_INT_ERR         = 0x03U;    /* Reset Error, Reset interrupt*/
...
void WICA_Handler(void)
{
    if (WICA_SFRS->STATUS.LINS != 0U){
        LINS_SFRS->CTRL.SLEEP = 0U;                     /*  wake up lins occurs when wake up signal detected  */
    ...
    if (WICA_SFRS->STATUS.TIMER != 0U){
        WICA_SFRS->CTRL.TIMERIRQCLR = 1U;
    }
    
    if (WICA_SFRS->STATUS.LINM != 0U){
        LINM_SFRS->CTRL.SLEEP = 0U;
...
static void LINS_SLAVE_ISR(void)
{
    ...
    if ((status & E_LIN_STATUS_DATA_REQ) != 0U){
        ...
        if (LINS_SFRS->STATUSEXT.COMPLETETX == 0U){ /*receive finished */
            if (index >= 0){
